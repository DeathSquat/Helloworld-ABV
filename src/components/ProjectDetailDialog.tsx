...
export const ProjectDetailDialog = ({ open, onOpenChange, resource }: ProjectDetailDialogProps) => {
  ...
  const handleStartProject = () => {
    ...
    const returnToAbsolute = `${window.location.origin}${location.pathname}${location.search}`;
    const returnTo = encodeURIComponent(returnToAbsolute);
    navigate(`/ide?project=${encodedTitle}&type=${projectType}&returnTo=${returnTo}`);
    onOpenChange(false);
  };
