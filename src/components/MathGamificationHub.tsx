...
import { getMathGameScores, MathGameScore } from '@/integrations/firebase/client';
...
const MathGamificationHub = () => {
  ...
  const [bestScoresByGameId, setBestScoresByGameId] = useState<Record<string, MathGameScore>>({});
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!user) {
        if (mounted) setBestScoresByGameId({});
        return;
      }

      try {
        const scores = await getMathGameScores(user.uid);
        if (mounted) setBestScoresByGameId(scores);
      } catch (e) {
        console.error('Failed to load math game scores:', e);
        if (mounted) setBestScoresByGameId({});
      }
    })();
  ...
  const gamesWithFirebaseScores = useMemo(() => {
    return mathGames.map((g) => {
      const best = bestScoresByGameId[g.id]?.bestScore;
      return {
        ...g,
        bestScore: best && best > 0 ? best : null,
      };
    });
  }, [bestScoresByGameId]);

  const filteredGames = selectedCategory === 'all' 
    ? gamesWithFirebaseScores 
    : gamesWithFirebaseScores.filter(game => game.category === selectedCategory);

  const totalPointsEarned = Number(userProfile?.total_xp ?? 0);
  const totalGamesCompleted = useMemo(() => {
    return Object.values(bestScoresByGameId).filter((s) => Number(s.bestScore ?? 0) > 0).length;
  }, [bestScoresByGameId]);

  const winStreak = Number(userProfile?.learning_streak ?? 0);
