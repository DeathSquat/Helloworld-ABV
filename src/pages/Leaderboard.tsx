...
import { getLeaderboardTopUsers, LeaderboardUser } from '@/integrations/firebase/client';
...
const Leaderboard = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const topUsers = await getLeaderboardTopUsers(10);
        if (mounted) {
  ...
  const currentUserRank = useMemo(() => {
    if (!user) return null;
    const idx = leaderboardUsers.findIndex((u) => u.id === user.uid);
    return idx >= 0 ? idx + 1 : null;
  }, [leaderboardUsers, user]);

  const currentUserPoints = Number(userProfile?.total_xp ?? 0);
  const currentUserStreak = Number(userProfile?.learning_streak ?? 0);
  const currentUserCourses = Number(userProfile?.completed_lessons ?? 0);
              ...
              <CardContent>
                <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-4 w-full">
                        ...
                        <div className="flex items-center gap-3">
                          <Avatar className="w-10 h-10">
                            <AvatarFallback className={`text-sm font-bold ${getRankBadgeColor(index + 1)}`}>
                              {lbUser.name
                                .split(' ')
                                .filter(Boolean)
                                .slice(0, 2)
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
                              <div className="flex items-center gap-1">
                                ...
                                <span>{lbUser.streak} day streak</span>
                              </div>
                            </div>
                          </div>
                        </div>
                  ...
                  {!loading && leaderboardUsers.length === 0 && (
                    <div className="p-4 text-sm text-muted-foreground">No leaderboard data yet.</div>
                  )}
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-gradient-to-br from-golden/10 to-golden/5 border-golden/20">
              ...
              <CardContent className="space-y-4">
                <div className="text-center">
                  <div className="text-3xl font-bold text-golden mb-1">{currentUserRank ? `#${currentUserRank}` : '—'}</div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-foreground">{currentUserPoints.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">Streak</span>
                    <span className="font-semibold text-foreground">{currentUserStreak} days</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">Courses</span>
                    <span className="font-semibold text-foreground">{currentUserCourses}</span>
                  </div>
                </div>
